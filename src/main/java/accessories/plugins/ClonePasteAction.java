...
public class ClonePasteAction extends MindMapNodeHookAdapter {
    ...
    public void invoke(MindMapNode pNode) {
        ...
        for (var copiedNode : mindMapNodes) {
    ...
    public void addNewClone(MindMapNode originalNode, MindMapNode pDestinationNode, Transferable copy) {
        ...
        log.trace("Original node {}, id {}", originalNode, originalNodeId);
        if (originalNode.isRoot()) {
            getMindMapController().getController().errorMessage(getMindMapController().getText("clone_plugin_no_root_cloning"));
            return;
        }

        // insert clone:
        List<MindMapNode> listOfChilds = pDestinationNode.getChildren();
        var listOfChildIds = new ArrayList<String>();
        for (var node : listOfChilds) {
            String nodeID = getMindMapController().getNodeID(node);
            listOfChildIds.add(nodeID);
            log.trace("Old child id:{}", nodeID);
        }
    ...
    public static class CloneProperties {
        ...
        private final HashSet<ClonePropertiesObserver> mObserverSet = new HashSet<>();
        ...
        public void setCloneItself(boolean pCloneItself) {
            log.trace("Setting mCloneItself to {}", pCloneItself);
            boolean fire = false;
    ...
    public static class Registration implements HookRegistration,
         ...
         */
        private final HashMap<String, HashSet<MindMapNode>> mCloneIdsMap = new HashMap<>();
        /**
         * This is the reverse of mCloneIdsMap: {@link MindMapNode} to cloneId.
         */
        private final HashMap<MindMapNode, String> mClonesMap = new HashMap<>();

        /**
         * This is a storage cloneId to clone properties.
         */
        private final HashMap<String, CloneProperties> mClonePropertiesMap = new HashMap<>();

        private final MindMapController controller;

        private final MindMap mMap;

        private final List<MindMapNode> mLastMarkedNodeViews = new ArrayList<>();
        ...
        public boolean registerClone(String pCloneId, ClonePlugin pPlugin) {
            ...
            for (Iterator<MindMapNode> it = v.iterator(); it.hasNext(); ) {
                ...
                if (otherClone == null) {
                    it.remove();
                    log.warn("Found clone node {} which isn't a clone any more.", controller.getNodeID(otherCloneNode));
                    continue;
                }
                // inform all others
        ...
        private XmlAction cloneAction(XmlAction doAction) {
            ...
            if (doAction instanceof NodeAction nodeAction) {
                ...
                if (doAction instanceof CompoundAction compoundAction) {
                    ...
                    int index = 0;
                    for (var subAction : choiceList) {
                        subAction = cloneAction(subAction);
                        CompoundAction.Choice choice = JIBXGeneratedUtil.choiceFromXmlActions(subAction);
                        compoundAction.getChoiceList().add(index, choice);
                        index++;
                    }
                }
            }
        ...
        private XmlAction cloneAction(NodeAction nodeAction, MindMapNode node) {
            ...
            for (var pair : correspondingNodes) {
        ...
        private void getNewCompoundAction(NodeAction nodeAction,
            ...
            CompoundAction.Choice copiedNodeActionChoice = JIBXGeneratedUtil.choiceFromXmlActions(copiedNodeAction);
            if (copiedNodeAction instanceof PasteNodeAction) {
                 ...
                 */
                compound.addChoice(copiedNodeActionChoice);
            } else {
                compound.getChoiceList().add(0, copiedNodeActionChoice);
            }
        }
        ...
        public void changeNodeListMember(
            ...
            if (!correspondingMoveNodes.isEmpty()) {
                ...
                for (var pair : correspondingMoveNodes) {
        ...
        public List<MindMapNodePair> getCorrespondingNodes(MindMapNode pNode,
            ...
            var indexVector = new ArrayList<Integer>();
            if (pStartWithParent) {
            ...
            var returnValue = new ArrayList<MindMapNodePair>();
            MindMapNode originalNode = child;
            HashSet<MindMapNode> targets = mCloneIdsMap.get(mClonesMap.get(child));
            CloneLoop:
            for (var cloneNode : targets) {
                MindMapNode target = cloneNode;
                if (cloneNode == originalNode)
                    continue;
                for (int i = indexVector.size() - 1; i >= 0; --i) {
                    int index = indexVector.get(i).intValue();
                    if (target.getChildCount() <= index) {
                        log.warn("Index {} in other tree not found from {} originating from {} start at parent {}", index, printNodeIds(targets), printNodeId(cloneNode), pStartWithParent);
        ...
        public void markShadowNode(NodeView pNode, boolean pEnableShadow) {
            ...
            if (!pEnableShadow) {
                if (!mLastMarkedNodeViews.isEmpty()) {
                    for (var node : mLastMarkedNodeViews) {
                        if (mClonesMap.containsKey(node)) {
        ...
        public void markShadowNode(MindMapNode model, boolean pEnableShadow) {
            ...
            try {
                ...
                for (var shadowNode : shadowNodes) {
