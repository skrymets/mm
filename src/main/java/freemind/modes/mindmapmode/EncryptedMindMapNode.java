...
public class EncryptedMindMapNode extends MindMapNodeModel {
    ...
    private MindMapNode getNodeFromXml(String childXml) throws IOException {
        ...
        MindMapNode node = getMap().loadTree(new FileTools.StringReaderCreator(childXml), MapAdapter.sDontAskInstance);
        return node;
    }
    ...
    public boolean checkPassword(StringBuilder givenPassword) {
        ...
        if (password != null) {
            if (!equals(givenPassword, password)) {
                log.warn("Wrong password supplied (cached!=given).");
                return false;
            }
            return true;
        }
        ...
        if (decryptedNode == null) {
            log.warn("Wrong password supplied (deciphered text is null).");
            return false;
        }
        if (!decryptedNode.startsWith("<node ")) {
            ...
            if (!HtmlTools.getInstance().isWellformedXml(decryptedNode)) {
                log.warn("Wrong password supplied (malformed deciphered text).");
                return false;
            }
        }
